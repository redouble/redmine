...
task :import_users => :environment do
  ...
  get_user_list(Time.now() - 1.days, s.value['shike_url']).each do |u|
    create_or_update_user(u)
  end
end

def get_user_list(lasttime, host)
  puts "Sync users from: #{lasttime.to_s}"
  begin
    JSON.parse( open(host + '/users/list.json?time=' + lasttime.strftime('%Y-%m-%d %H:%M:%S')).read )
  rescue
